...
import { createPool, incrementBudget, getBudget, ensureBudgetRow, getCrawlPage, setCrawlPage, upsertObservations, logEvent, } from "./db.js";
...
async function fetchOnePage(browser, market, seedUrl, pageNum) {
    ...
    try {
        ...
        const rows = await page.evaluate((pageUrlArg) => {
            ...
            for (const card of cards) {
                ...
                for (const a of links) {
                    ...
                    if (!isImg && t.length > 5 && !t.toLowerCase().startsWith("shop on ebay"))
                        title = t;
                }
                ...
                if (!cleanId || !href || title.length < 5)
                    continue;
                out.push({
                    itemId: cleanId,
                    title,
