...
import { addExtraInitScript, humanHoverSomeCards, humanPointerWander, humanReadingPause, humanScrollResults, humanSkimHome, } from "./human.js";
import { createPool, ensureSchema, incrementBudget, getBudget, ensureBudgetRow, getCrawlPage, setCrawlPage, upsertObservations, logEvent, } from "./db.js";
...
const UK_SEED_DEFAULT = "https://www.ebay.co.uk/sch/i.html?_nkw=pokemon+card&LH_Sold=1&LH_Complete=1&_ipg=60&rt=nc";
...
const ukSeedEnvHadSacat0 = Boolean(ukSeedEnv) &&
...
function newContextOptions(market) {
...
function botWall(html) {
    const h = html.toLowerCase();
    return (h.includes("pardon our interruption") ||
...
function buildSearchUrl(seed, pageNum) {
...
async function extractPriceFromItemDom(pricePage) {
    return pricePage.evaluate(() => {
        ...
        const ogCur = document.querySelector('meta[property="og:price:currency"]')?.getAttribute("content")?.trim() || "";
        ...
        if (ipText.length > 0 && ipText.length < 80 && /[\d£$€]/.test(ipText))
            return ipText;
        for (const sel of [
            ...
            if (t.length > 0 && t.length < 120 && /[\d£$€.,]/.test(t))
                return t;
        }
...
async function extractPriceFromItemOcr(page, worker, market) {
    await page
    ...
    if (market === "uk")
        blob = blob.replace(/^[Ll](?=\s*[\d,.])/g, "£");
    const pick = blob.match(/[\u00A3£]\s*[\d,]+(?:\.\d{1,2})?/) ||
        blob.match(/\$\s*[\d,]+(?:\.\d{1,2})?/) ||
        blob.match(/€\s*[\d,]+(?:\.\d{1,2})?/);
    if (pick)
        return pick[0].replace(/\s+/g, " ").trim();
...
async function enrichMissingPricesFromItemPages(ctx, rows, searchReferer, market) {
    let enriched = 0;
    let attempted = 0;
    let ocrFilled = 0;
    let ocrWorker = null;
    ...
    try {
        for (const row of rows) {
            if (row.priceText?.trim())
                continue;
            if (ITEM_PRICE_ENRICH_MAX > 0 && attempted >= ITEM_PRICE_ENRICH_MAX)
                break;
            attempted++;
            try {
...
async function fetchOnePage(browser, market, seedUrl, pageNum) {
    ...
    try {
        ...
        const { rows, diag } = await page.evaluate((pageUrlArg) => {
            ...
            const allItmLinks = Array.from(document.querySelectorAll('a[href*="/itm/"]'));
...
async function main() {
    ...
    let lastMarket = "uk";
    const badUkSacat0 = UK_SEED.includes("ebay.co.uk") && (UK_SEED.includes("_sacat=0") || UK_SEED.includes("_sacat=0&"));
    console.log(JSON.stringify({
        msg: "crawler_start",
    ...
    while (true) {
        ...
        try {
            ...
            await sleep(randBetween(isBotWall ? BOT_WALL_BACKOFF_MIN_MS : PAGE_ERR_BACKOFF_MIN_MS, isBotWall ? BOT_WALL_BACKOFF_MAX_MS : PAGE_ERR_BACKOFF_MAX_MS));
        }
