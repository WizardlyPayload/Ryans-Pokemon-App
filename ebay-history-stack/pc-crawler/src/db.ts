...
export async function upsertPcProduct(
  pool: pg.Pool,
  row: {
    pcProductId: string;
    slug: string;
  ...
  await pool.query(
    `INSERT INTO pc_price_snapshots (pc_product_id, tiers, extras, parse_version)
     VALUES ($1::bigint, $2::jsonb, $3::jsonb, $4)`,
    [row.pcProductId, JSON.stringify(row.tiers), JSON.stringify(row.extras), row.parseVersion],
  );
}
